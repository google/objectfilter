...
our fleet like this:
  ...
  compiled_filter = parser.Compile(LowercaseAttributeFilterImplementation)
...
"tyres.brand", and a filter query such as:
    tyres.brand is "AVON"
...
filter is easy. Three basic filter implementations are given:
  ...
  LowercaseAttributeFilterImplementation: search path expansion is done on the
  lowercase version of search path compoenents, so that it only accesses
...
# TODO(nop): Change to an N-ary Operator?
...
class Context(Operator):
  ...
  def Matches(self, obj):
    logging.debug("Expanding context: %s", self.context)
    for object_list in self.value_expander.Expand(obj, self.context):
      for sub_object in object_list:
        logging.debug("Trying to match condition: '%s' on '%s'",
                      self.condition,
                      sub_object)
...
class Parser(lexer.SearchParser):
  ...
  tokens = [
      ...
      lexer.Token("ANDOR", r"(?i)(and|or|\&\&|\|\|)",
                  "PushState,BinaryOperator", "INITIAL"),
      lexer.Token("ANDOR", r"\)", "BracketClose", "ANDOR"),
  ...
  def Reduce(self):
    ...
    # Check for sanity
    if self.state != "INITIAL" and self.state != "ANDOR":
    ...
    logging.debug("Reduced result: %s", self.stack[0])
    return self.stack[0]
  ...
  def _CombineBinaryExpressions(self, operator):
    for i in range(1, len(self.stack)-1):
      ...
      if (isinstance(item, lexer.BinaryExpression) and
        ...
        self.stack[i+1] = None
        logging.debug("i %d", i)
        logging.debug("BinaryExpression: %s", self.stack[i])
  ...
  def _CombineContext(self):
    ...
    for i in range(0, len(self.stack)-1):
      ...
      if (isinstance(item, ContextExpression) and
        ...
        context.SetExpression(self.stack[i+1])
        self.stack[i+1] = None
        logging.debug("i %d", i)
        logging.debug("ContextExpression: %s", self.stack[i-1])
        logging.debug("self.stack[i-1] = %s", self.stack[i-2])
